...
describe('project.service', () => {
    describe('createProject', () => {
        it('creates a project with the given data', async () => {
            ...
            const result = await (0, project_service_1.createProject)(data);
    ...
    describe('getProjectById', () => {
        it('returns project with workBreakdownItems', async () => {
            ...
            const result = await (0, project_service_1.getProjectById)('proj-1');
    ...
    describe('updateProject', () => {
        it('updates allowed fields', async () => {
            ...
            const result = await (0, project_service_1.updateProject)('proj-1', { name: 'Updated' });
    ...
    describe('lockProjectStructure', () => {
        it('sets structureLocked to true', async () => {
            ...
            const result = await (0, project_service_1.lockProjectStructure)('proj-1');
            expect(mockProjectUpdate).toHaveBeenCalledWith({
                where: { id: 'proj-1' },
        ...
        it('throws if project not found', async () => {
            ...
            await expect((0, project_service_1.lockProjectStructure)('proj-1')).rejects.toThrow('Project not found');
        });
    });
    describe('addWorkBreakdownItem', () => {
        it('adds item and locks structure on first item', async () => {
            ...
            const result = await (0, project_service_1.addWorkBreakdownItem)('proj-1', {
                code: '1.1',
                name: 'Foundation',
            });
        ...
        it('does not lock structure on subsequent items', async () => {
            ...
            mockWorkBreakdownItemCreate.mockResolvedValue(item);
            const result = await (0, project_service_1.addWorkBreakdownItem)('proj-1', { code: '1.2', name: 'Walls' });
            expect(mockProjectUpdate).not.toHaveBeenCalled();
            expect(result).toEqual(item);
        });
        it('throws if project not found', async () => {
            mockProjectFindUnique.mockResolvedValue(null);
            await expect((0, project_service_1.addWorkBreakdownItem)('proj-1', { code: '1.1', name: 'X' })).rejects.toThrow('Project not found');
        });
        it('throws if structureType not set', async () => {
            mockProjectFindUnique.mockResolvedValue({ id: 'proj-1', structureType: null });
            await expect((0, project_service_1.addWorkBreakdownItem)('proj-1', { code: '1.1', name: 'X' })).rejects.toThrow('Project structure type must be set before adding work breakdown items');
        });
    });
    describe('deleteProject', () => {
        it('soft deletes by setting status to cancelled', async () => {
            const updated = { id: 'proj-1', status: status_1.PROJECT_STATUSES.CANCELLED };
            mockProjectUpdate.mockResolvedValue(updated);
            const result = await (0, project_service_1.deleteProject)('proj-1');
            expect(mockProjectUpdate).toHaveBeenCalledWith({
                where: { id: 'proj-1' },
    ...
    describe('setProjectOrgBridge', () => {
        it('sets orgId on project', async () => {
            ...
            mockProjectUpdate.mockResolvedValue(updated);
            const result = await (0, project_service_1.setProjectOrgBridge)('proj-1', 'org-2');
            expect(mockProjectUpdate).toHaveBeenCalledWith({
                where: { id: 'proj-1' },
                data: { orgId: 'org-2' },
            });
