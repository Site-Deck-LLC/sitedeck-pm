...
describe('communications.service', () => {
    describe('RFI log', () => {
        ...
        it('updateRfi updates fields without status change', async () => {
            ...
            await Promise.resolve().then(() => __importStar(require('./communications.service'))).then((m) => m.updateRfi('rfi-1', { ballInCourt: 'EOR' }));
            expect(mockRfiUpdate).toHaveBeenCalledWith(expect.objectContaining({
                where: { id: 'rfi-1' },
        ...
        it('updateRfi appends history when status changes', async () => {
            ...
            await Promise.resolve().then(() => __importStar(require('./communications.service'))).then((m) => m.updateRfi('rfi-1', { status: 'submitted' }));
            const call = mockRfiUpdate.mock.calls[0][0];
            const history = call.data.statusHistory;
    ...
    describe('Meeting minutes', () => {
        it('creates a meeting with all fields', async () => {
            ...
            const result = await (0, communications_service_1.createMeeting)({
                projectId: 'proj-1',
                title: 'Weekly OAC',
        ...
        it('returns meetings for a project ordered by date desc', async () => {
            ...
            const result = await (0, communications_service_1.getMeetingsByProject)('proj-1');
        ...
        it('filters meetings by date range when provided', async () => {
            ...
            await (0, communications_service_1.getMeetingsByProject)('proj-1', start, end);
            expect(mockMeetingFindMany).toHaveBeenCalledWith({
                where: {
                    projectId: 'proj-1',
        ...
        it('returns a single meeting by id', async () => {
            ...
            const result = await (0, communications_service_1.getMeetingById)('meet-1');
        ...
        it('updates meeting fields', async () => {
            ...
            const result = await (0, communications_service_1.updateMeeting)('meet-1', { status: 'published' });
        ...
        it('updates a single action item status by index', async () => {
            ...
            await (0, communications_service_1.updateMeetingActionItemStatus)('meet-1', 1, 'closed');
        ...
        it('throws when action item index is out of range', async () => {
            ...
            await expect((0, communications_service_1.updateMeetingActionItemStatus)('meet-1', 5, 'closed')).rejects.toThrow('Action item index out of range');
        });
        it('throws when updating action item on non-existent meeting', async () => {
            mockMeetingFindUnique.mockResolvedValue(null);
            await expect((0, communications_service_1.updateMeetingActionItemStatus)('meet-x', 0, 'closed')).rejects.toThrow('Meeting not found');
        });
        it('deletes a meeting', async () => {
            mockMeetingDelete.mockResolvedValue({ id: 'meet-1' });
            await (0, communications_service_1.deleteMeeting)('meet-1');
        ...
        it('returns meeting PDF data with project name', async () => {
            ...
            const result = await (0, communications_service_1.getMeetingPdfData)('meet-1');
        ...
        it('throws when PDF data requested for non-existent meeting', async () => {
            mockMeetingFindUnique.mockResolvedValue(null);
            await expect((0, communications_service_1.getMeetingPdfData)('meet-x')).rejects.toThrow('Meeting not found');
        });
    });
});
