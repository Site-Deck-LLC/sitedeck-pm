...
describe('integration.service', () => {
    ...
    describe('voiceToIssue', () => {
        it('records the voice memo and returns a pending status with the memoId', async () => {
            ...
            const result = await (0, integration_service_1.voiceToIssue)({
                projectId: 'proj-1',
                audioUrl: 'https://storage/voice.m4a',
                durationSeconds: 12,
                createdBy: 'user-1',
            });
            expect(mockVoiceMemoCreate).toHaveBeenCalledWith(expect.objectContaining({
                data: expect.objectContaining({
                    projectId: 'proj-1',
                    status: 'pending',
                    createdBy: 'user-1',
    ...
    describe('appendIssueNote', () => {
        it('appends a new note to an existing issue with empty notes', async () => {
            ...
            await (0, integration_service_1.appendIssueNote)({ issueId: 'iss-1', text: 'hello', author: 'u1' });
            expect(mockIssueUpdate).toHaveBeenCalledWith(expect.objectContaining({
                where: { id: 'iss-1' },
                data: expect.objectContaining({
        ...
        it('appends a new note to existing notes (preserves order)', async () => {
            ...
            await (0, integration_service_1.appendIssueNote)({ issueId: 'iss-1', text: 'third', author: 'u3' });
            const call = mockIssueUpdate.mock.calls[0][0];
            const notes = call.data.notes;
        ...
        it('throws when the issue does not exist', async () => {
            mockIssueFindUnique.mockResolvedValue(null);
            await expect((0, integration_service_1.appendIssueNote)({ issueId: 'nonexistent', text: 'x', author: 'u1' })).rejects.toThrow('Issue not found');
        });
    });
});
