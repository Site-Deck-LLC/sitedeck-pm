...
describe('procurement.service', () => {
  describe('PO lifecycle', () => {
    ...
    it('creates a PO with line items and computed total', async () => {
      ...
      expect(mockPurchaseOrderCreate).toHaveBeenCalledWith(
    ...
    it('gets POs by project', async () => {
      ...
      const result = await getPurchaseOrdersByProject('proj-1');
      expect(mockPurchaseOrderFindMany).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it('issues a PO', async () => {
      ...
      const result = await issuePurchaseOrder('po-1');
    ...
    it('closes a PO', async () => {
      ...
      const result = await closePurchaseOrder('po-1');
      expect(mockPurchaseOrderUpdate).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { id: 'po-1' },
          data: { status: 'closed' },
  ...
  describe('addLineItemToPO', () => {
    ...
    it('adds a line item and updates total', async () => {
      ...
      const result = await addLineItemToPO('po-1', {
        materialName: 'Rebar',
        quantity: 30,
        unit: 'ft',
        unitPrice: 100,
    ...
    it('throws if PO not found', async () => {
      ...
      await expect(
        addLineItemToPO('po-1', { materialName: 'X', quantity: 1, unit: 'ea', unitPrice: 1 })
  ...
  describe('recordMaterialDelivery', () => {
    ...
    it('creates a delivery and updates line item quantityReceived', async () => {
      ...
      const result = await recordMaterialDelivery({
        projectId: 'proj-1',
        poId: 'po-1',
        lineItemId: 'li-1',
        quantityReceived: 30,
      ...
      expect(mockPurchaseOrderLineItemUpdate).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { id: 'li-1' },
    ...
    it('is idempotent when deliveryReference is provided', async () => {
      ...
      expect(mockMaterialDeliveryFindFirst).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  describe('getDeliveries', () => {
    ...
    it('gets deliveries by PO', async () => {
      ...
      const result = await getDeliveriesByPO('po-1');
      expect(mockMaterialDeliveryFindMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { poId: 'po-1' },
    ...
    it('gets deliveries by project', async () => {
      mockMaterialDeliveryFindMany.mockResolvedValue([{ id: 'del-1' }]);
      const result = await getDeliveriesByProject('proj-1');
      expect(mockMaterialDeliveryFindMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { projectId: 'proj-1' },
          orderBy: { receivedAt: 'desc' },
  ...
  describe('3-way match', () => {
    ...
    it('passes 3-way match when all checks align', async () => {
      ...
      const result = await performThreeWayMatch('inv-1');
    ...
    it('throws when invoice not found for match', async () => {
      ...
      await expect(performThreeWayMatch('inv-1')).rejects.toThrow('Invoice not found');
    });
    ...
    it('throws when approving failed match', async () => {
      ...
      await expect(approveInvoice('inv-1')).rejects.toThrow(
        'Cannot approve invoice with failed 3-way match'
      );
    });
    ...
    it('throws when approving before match is performed', async () => {
      ...
      await expect(approveInvoice('inv-1')).rejects.toThrow(
        '3-way match must be performed before approval'
      );
    });

    it('gets invoices by project', async () => {
      mockInvoiceFindMany.mockResolvedValue([{ id: 'inv-1' }]);
      const result = await getInvoicesByProject('proj-1');
    ...
    it('gets invoices by PO', async () => {
      mockInvoiceFindMany.mockResolvedValue([{ id: 'inv-1' }]);
      const result = await getInvoicesByPO('po-1');
      expect(mockInvoiceFindMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { poId: 'po-1' },
          orderBy: { createdAt: 'desc' },
  ...
  describe('48-hour alert logic', () => {
    ...
    it('returns green when no POs linked to activities', async () => {
      ...
      const result = await getMaterialsAlertStatus('proj-1');
  ...
  describe('subcontract management', () => {
    ...
    it('creates a subcontract', async () => {
      ...
      expect(mockSubcontractCreate).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({
            subcontractorName: 'ABC Concrete',
            contractAmount: 50_000,
    ...
    it('gets a subcontract by id', async () => {
      ...
      const result = await getSubcontractById('sub-1');
    ...
    it('gets subcontracts by project', async () => {
      ...
      const result = await getSubcontractsByProject('proj-1');
      expect(mockSubcontractFindMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { projectId: 'proj-1' },
          orderBy: { createdAt: 'desc' },
        })
    ...
    it('updates subcontract status', async () => {
      ...
      const result = await updateSubcontractStatus('sub-1', 'completed');
      expect(mockSubcontractUpdate).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { id: 'sub-1' },
    ...
    it('throws on invalid subcontract status', async () => {
      await expect(updateSubcontractStatus('sub-1', 'invalid')).rejects.toThrow(
        'Invalid subcontract status: invalid'
      );
    });
    ...
    it('records progress billing to schedule of values', async () => {
      ...
      const result = await recordProgressBilling('sub-1', 20_000, 'Phase 1');
      expect(mockSubcontractUpdate).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { id: 'sub-1' },
          data: {
      ...
      const sov = result.scheduleOfValues as { description: string; value: number }[];
    ...
    it('throws when recording billing for non-existent subcontract', async () => {
      ...
      await expect(recordProgressBilling('sub-1', 5_000, 'X')).rejects.toThrow(
